...
import { retry, retrieveEnvVariable, retrieveTokenValueByAddress, getRaydiumPoolPrice } from './utils';
...
async function getTokenInfo(mint: PublicKey): Promise<{ name: string; symbol: string }> {
  try {
    const metadata = await getTokenMetadata(solanaConnection, mint);
    return {
      name: metadata?.name || 'Unknown Token',
      symbol: metadata?.symbol || 'UNKNOWN'
    };
  } catch (error) {
    logger.warn({ mint, error }, 'Failed to get token metadata, using default values');
...
async function buy(accountId: PublicKey, accountData: LiquidityStateV4): Promise<void> {
  try {
    ...
    if (!confirmation.value.err) {
      const investmentSOL = Number(quoteAmount.toFixed());

      const buyTransaction: TokenTransaction = {
        timestamp: new Date().toISOString(),
        mintAddress: accountData.baseMint.toString(),
        action: 'BUY',
        investmentSOL,
        profitPercentage: 0
      };
      transactionHistory.push(buyTransaction);
      ...
      logger.info(
        {
          ...
          investmentSOL,
          priceSOL: tokenAccount.buyValue
        },
        `Bought at ${tokenAccount.buyValue} SOL`
      );
    } else {
...
async function monitorPrice(tokenAccount: MinimalTokenAccountData, accountData: { mint: PublicKey; amount: BigNumberish }, tokenInfo: { name: string; symbol: string }) {
  try {
    const currentTokenAccount = existingTokenAccounts.get(accountData.mint.toString());
    if (!currentTokenAccount || !currentTokenAccount.poolKeys) {
      logger.info({ mint: accountData.mint }, 'Token no longer in wallet or pool keys missing, stopping monitoring');
      return true;
    }

    let currValue = null;
    let retries = 0;
    while (!currValue && retries < 3) {
      try {
        currValue = await getRaydiumPoolPrice(solanaConnection, currentTokenAccount.poolKeys);
      } catch (error) {
        retries++;
        if (retries === 3) {
          logger.warn({ mint: accountData.mint }, 'Unable to get pool price after retries');
          return false;
        }
    ...
    const netChange = (currValue - tokenAccount.buyValue!) / tokenAccount.buyValue!;
    const amountNumber = Number(accountData.amount.toString());
    const currentValueSOL = currValue * amountNumber;

    logger.info(
      { 
        mint: accountData.mint
      },
      `Current Price: ${currValue} SOL | Net Change: ${(netChange * 100).toFixed(2)}% | Current Value: ${currentValueSOL.toFixed(4)} SOL`
    );

    if (netChange >= TAKE_PROFIT || netChange <= -STOP_LOSS) {
      logger.info(
        { mint: accountData.mint },
        `${netChange >= TAKE_PROFIT ? 'Take Profit' : 'Stop Loss'} triggered at ${(netChange * 100).toFixed(2)}%`
      );
      const sold = await sell(currentTokenAccount.address, accountData.mint, accountData.amount, currValue);
      if (sold) {
        existingTokenAccounts.delete(accountData.mint.toString());
        logger.info({ mint: accountData.mint }, 'Token sold successfully, removed from monitoring');
        return true;
      }
    }
    return false;
  } catch (error) {
    logger.error({ mint: accountData.mint, error }, 'Error monitoring price');
    return false;
  }
}
...
async function sell(accountId: PublicKey, mint: PublicKey, amount: BigNumberish, value: number): Promise<boolean> {
  ...
  do {
    try {
      ...
      const profitPercentage = ((value - tokenAccount.buyValue!) / tokenAccount.buyValue!) * 100;

      const sellTransaction: TokenTransaction = {
        timestamp: new Date().toISOString(),
        mintAddress: mint.toString(),
        action: 'SELL',
        investmentSOL: tokenAccount.buyValue! * Number(amount.toString()),
        profitPercentage
      };
      transactionHistory.push(sellTransaction);
      ...
      logger.info(
        ...
        `Sold at ${value} SOL | Profit: ${profitPercentage.toFixed(2)}%`
      );
      return true;
    } catch (e: any) {
      retries++;
  ...
  if (AUTO_SELL) {
    const walletSubscriptionId = solanaConnection.onProgramAccountChange(
      ...
      async (updatedAccountInfo) => {
        ...
        if (!tokenAccount || !tokenAccount.buyValue || !tokenAccount.poolKeys) {
          return;
        }

        // Monitor price continuously until TP/SL is hit
        const monitorPrice = async () => {
          try {
            ...
            const currValue = await getRaydiumPoolPrice(solanaConnection, currentTokenAccount.poolKeys);
            if (!currValue) {
              logger.warn({ mint: accountData.mint }, 'Unable to get pool price, will retry');
              return false;
            }

            const netChange = (currValue - tokenAccount.buyValue!) / tokenAccount.buyValue!;
            const amountNumber = Number(accountData.amount);
            const currentValueSOL = currValue * amountNumber;
        ...
        // Start monitoring with a reasonable interval
        const monitorInterval = setInterval(async () => {
          const shouldStop = await monitorPrice();
          if (shouldStop) {
...
function saveTransactionHistory() {
  const history = JSON.stringify(transactionHistory, null, 2);
  fs.writeFileSync('transaction_history.json', history);
  logger.info('Transaction history saved to transaction_history.json');
}
