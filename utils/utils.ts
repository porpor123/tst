...
export const getRaydiumPoolPrice = async (
  connection: Connection,
  poolKeys: LiquidityPoolKeys
): Promise<number | undefined> => {
  try {
    const [baseVaultBalance, quoteVaultBalance] = await Promise.all([
      connection.getTokenAccountBalance(poolKeys.baseVault),
      connection.getTokenAccountBalance(poolKeys.quoteVault)
    ]);

    if (!baseVaultBalance.value?.uiAmount || !quoteVaultBalance.value?.uiAmount) {
      logger.warn('Unable to get vault balances');
      return undefined;
    }

    // Calculate price from pool balances
    const price = quoteVaultBalance.value.uiAmount / baseVaultBalance.value.uiAmount;
    return price;
  } catch (error) {
    logger.error({ error }, 'Failed to get Raydium pool price');
    return undefined;
  }
};
